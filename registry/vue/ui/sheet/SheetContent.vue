...
const delegatedProps = reactiveOmit(props, "class", "side", "size", "closeLabel")
...
<template>
  <DialogPortal>
    ...
    <DialogContent
      ...
      :data-size="size"
      :class="cn(
        'bg-card dark:bg-background data-[state=open]:animate-in data-[state=closed]:animate-out fixed z-50 flex flex-col gap-4 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
        side === 'right'
        ...
        (side === 'left' || side === 'right') && sizes[size],
        props.class)"
