...
// Saqara: `readOnly` locks a valid value legibly (disabled = unavailable, readOnly = locked but valid).
const props = withDefaults(
  defineProps<SelectTriggerProps & { class?: HTMLAttributes["class"], size?: "sm" | "default", readOnly?: boolean }>(),
  { size: "default", readOnly: false },
)

const delegatedProps = reactiveOmit(props, "class", "size", "readOnly", "disabled")
...
<template>
  <SelectTrigger
    ...
    :class="cn(
      ...
      'data-[readonly]:cursor-default data-[readonly]:bg-muted/40 data-[readonly]:opacity-100 dark:data-[readonly]:bg-muted/40',
      props.class,
    )"
  >
    <slot />
    <SelectIcon v-if="!readOnly" as-child>
